...
import CommunityOpponentBrowser from "../components/CommunityOpponentBrowser.jsx";
...
export default function BattleFlow({ user, profile, mode, preselectedFighterId, singlePlayerMode: initialSinglePlayerMode, onNavigate }) {
  ...
  const idempotencyKeyRef = useRef(getPendingBattle("battleFlow")?.key || null);
  ...
  const handleStart = async () => {
    ...
    setSaveFailed(false);
    setStarting(true);
    // Same key on every retry of this exact attempt — makes retries and
    // network hiccups safe instead of risking a duplicate save/ribbon.
    if (!idempotencyKeyRef.current) {
      idempotencyKeyRef.current = generateIdempotencyKey(user.id);
      savePendingBattle("battleFlow", idempotencyKeyRef.current, { note: "battle in progress" });
    }
    ...
    try {
      ...
      if (battleFormat === "gauntlet") {
        ...
        const { result, iWon, totalRibbons } = await executeGauntletBattle({
          ...
          lineupVisibility, idempotencyKey
        });
        ...
        idempotencyKeyRef.current = null; clearPendingBattle("battleFlow");
      ...
      const { result, iWon, totalRibbons } = await executeBattle({
        user, profile, myTeam, opponentTeam, battleMode: battleSize, battleType, opponentUserId, idempotencyKey
      });
      ...
      // The battle itself may have already run — the idempotency key stays
      // the same so tapping Retry Save never records it twice.
      setSaveFailed(true);
      setError(e.message || "Battle completed, but progress could not be saved.");
      setStarting(false);
    }
  };
        ...
        <>
          ...
          {mode === "computer" && singlePlayerMode === "community" && myFighterIds.length === requiredCount && (
            <div className="card">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button className="btn btn-primary" style={{ flex: 1 }} onClick={() => loadCommunityOpponents([])} disabled={loadingCommunity}>
                  {loadingCommunity ? "Loading..." : "Random Opponent"}
                </button>
                <button className="btn btn-cyan" style={{ flex: 1 }} onClick={() => setShowBrowser(true)}>
                  Browse Opponents
                </button>
              </div>
              {communityBuilds && (
          ...
          {showBrowser && (
            <CommunityOpponentBrowser
              userId={user.id}
              requiredCount={requiredCount}
              onConfirm={(selected) => { setCommunityBuilds(selected); setShowBrowser(false); }}
