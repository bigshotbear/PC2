  ...
  return (
    <div className="page">
      ...
      <div className="hero-header" style={{ padding: "18px 16px", marginBottom: 16 }}>
        <div className="hero-name" style={{ fontSize: 22 }}>Battle History</div>
        <div style={{ color: "var(--blue-bright)", fontSize: 12.5, position: "relative" }}>Every fight, tracked and permanent.</div>
      </div>
      ...
      {loading ? (
        ...
        battles.map((b) => {
          ...
          return (
            <button key={b.id} className={`card ${iWon ? "card-success" : "card-danger"}`} style={{ width: "100%", textAlign: "left" }} onClick={() => setSelected(b)}>
