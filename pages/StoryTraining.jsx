...
export default function StoryTraining({ user, fighterId, onNavigate }) {
  ...
  const handleRetryLevel = async () => {
    await updateStoryProgress(progress.id, {
      run_status: "active", total_attempts: (progress.total_attempts || 0) + 1
      // current_level is left untouched — it already holds the level that
      // was lost, which is exactly what Retry This Level should resume at.
    });
    onNavigate("storyLevel", { fighterId });
  };

  const handleTrainAndReturn = async () => {
    // Mark the training claim as settled (idle) but leave current_level
    // exactly as-is — that's the checkpoint StoryHome will offer to retry.
    await updateStoryProgress(progress.id, { run_status: "idle" });
    ...
    <div className="page">
      ...
      {claimed || alreadyClaimed ? (
        <>
          ...
          <button className="btn btn-primary" onClick={handleRetryLevel}>
            Retry Level {progress.current_level}
          </button>
          <button className="btn btn-cyan" onClick={handleTrainAndReturn}>
            Train and Return Later
          </button>
          <div style={{ fontSize: 11, color: "var(--text-dim)", margin: "6px 0 2px" }}>
            Want a completely fresh run instead? Use Reset Run below.
          </div>
        </>
      ) : !mode ? (
