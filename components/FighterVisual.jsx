...
export default function FighterVisual({
  ...
  const stateOffset = { idle: 0, attack: 18, hit: 0, ultimate: 0, defeat: 0, victory: 0, block: 0 }[state] || 0;
  const bodyOpacity = state === "defeat" ? 0.3 : state === "hit" ? 0.55 : 1;
  const bodyFilter = state === "hit" ? "brightness(1.9)" : state === "victory" ? "brightness(1.25) saturate(1.3)" : "none";
  ...
  const stateAnimation = {
    idle: animated ? "pcFloat 3s ease-in-out infinite" : "none",
    attack: "pcAttackLunge .35s ease-out",
    hit: "pcHitShake .3s ease-in-out",
    victory: "pcVictoryBounce 1s ease-in-out infinite",
    block: "pcBlockFlash .3s ease-out",
    ultimate: "pcFloat 1.2s ease-in-out infinite",
    defeat: "none"
  }[state] || "none";
  ...
  return (
    ...
    >
      <style>{`
        ...
        @keyframes pcAttackLunge {
          0% { transform: scaleX(${flip}) translateX(0); }
          45% { transform: scaleX(${flip}) translateX(${22 * flip}px); }
          100% { transform: scaleX(${flip}) translateX(0); }
        }
        @keyframes pcHitShake {
          0%,100% { transform: scaleX(${flip}) translateX(0); }
          20% { transform: scaleX(${flip}) translateX(${-8 * flip}px); }
          40% { transform: scaleX(${flip}) translateX(${6 * flip}px); }
          60% { transform: scaleX(${flip}) translateX(${-4 * flip}px); }
          80% { transform: scaleX(${flip}) translateX(${2 * flip}px); }
        }
        @keyframes pcVictoryBounce {
          0%,100% { transform: scaleX(${flip}) translateY(0) scale(1); }
          30% { transform: scaleX(${flip}) translateY(-10px) scale(1.03); }
          50% { transform: scaleX(${flip}) translateY(0) scale(1); }
        }
        @keyframes pcBlockFlash {
          0% { transform: scaleX(${flip}) scale(1); }
          50% { transform: scaleX(${flip}) scale(0.94); }
          100% { transform: scaleX(${flip}) scale(1); }
        }
      `}</style>
